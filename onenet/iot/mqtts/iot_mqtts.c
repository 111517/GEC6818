...
#define IOT_MQTTS_SERVER_ADDR_TLS "mqttstls.heclouds.com"
...
#define IOT_MQTTS_SERVER_ADDR "mqtts.heclouds.com"
...
static void iot_mqtts_message_arrived(void* usr_data, const uint8_t* topic, struct mqtt_message_t* message)
{

    ont_mqtt_t*          ont_mqtt = (ont_mqtt_t*)usr_data;
    ...
    if (NULL != (tmp_ptr = osl_strstr(topic, (const uint8_t*)"$sys"))) {
        if (NULL != (tmp_ptr = osl_strstr(topic, (const uint8_t*)"/cmd/request/")))    // data cmd
        {
            ...
            cmd_data.resp_len = 0;
            if (ERR_OK == ont_mqtt->evt_cb(IOT_DEV_CMD_RECEIVED, &cmd_data, sizeof(struct iot_dev_cmd_t))) {
...
ont_mqtt_t* iot_mqtts_login(struct iot_cloud_config_t* ont_cfg, ont_dev_event_cb event_cb, uint32_t timeout_ms)
{
    ...
    CHECK_EXPR_GOTO(ERR_OK != ret, _ERROR3, "mqtt_subscribe failed(%d)!", ret);
...
int32_t iot_mqtts_step(ont_mqtt_t* ont_mqtt, uint32_t timeout_ms)
{
    ...
    if ((ERR_OK == ret) && (0 <= ont_mqtt->cmd_resp_len)) {
